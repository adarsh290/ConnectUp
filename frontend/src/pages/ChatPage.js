...
const ChatPage = () => {
  ...
  // Ensure socket is initialized and keep ref in sync
  useEffect(() => {
    if (token) {
      if (!getSocket()) {
  ...
  // Keep isTypingRef in sync with state (for use in cleanup closures)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (socket && selectedUser) {
        ...
        // Use ref to check typing state — avoids putting isTyping in deps
  ...
  const setupSocketListeners = () => {
    ...
    if (socket) {
      ...
      socket.on('messages_read', (data) => {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.sender._id?.toString() === user.id?.toString() && !msg.read
              ? { ...msg, read: true }
              : msg
          )
        );
